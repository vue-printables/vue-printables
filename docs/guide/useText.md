...
interface TextConfigs {
  ...
  fill?: string;
  stroke?: string;
  [key: string]: any; // Allows additional [Fabric.js](https://fabricjs.com/) text properties
}
```
...
<template>
  <div class="text-editor">
    ...
    <div class="form-group">
      <label>Color:</label>
      <input type="color" v-model="textConfig.fill" @input="updateIfEditing" />
    </div>
    ...
    <button @click="addText(textConfig)" v-if="!editing">Add Text</button>
  </div>
</template>
...
const editing = computed(() => canvasState.activeObj.value?.type === "text");
